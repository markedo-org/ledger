...
func main() {
	...
	bootOwner := flag.String("boot-owner", "acme", "owner slug created on empty database")
	bootLedger := flag.String("boot-ledger", "inbox", "ledger slug created on empty database")
	bootActor := flag.String("boot-actor", "ada", "actor bound to the boot token")
